...
public class ReservedRoomsRepo implements AutoCloseable{
    ...
    public void rRInsert(RREntity entity) throws Exception{
        preparedStatement=connection.prepareStatement("insert into  reservedrooms (nationalcode,pfullname,mobile,ipay,resdays,rdatetime,roomno) values (?,?,?,?,?,?,?)");
    ...
    public  void  rRUpdate(RREntity entity,String originalNationalCode,int originalroomNo) throws Exception{
        preparedStatement=connection.prepareStatement("update reservedrooms set nationalcode=?,pfullname=?,mobile=?,ipay=?,resdays=?,rdatetime=?,roomno=? where nationalcode=? and roomno=?");
    ...
    public void rRdelete(String nationalCode,int roomNo)throws Exception{
        preparedStatement=connection.prepareStatement("delete from  reservedrooms where nationalcode=? and roomno=?");
    ...
    public List<RREntity> rRfullSelct()throws  Exception {
        preparedStatement = connection.prepareStatement("select roomno,nationalcode,pfullnamee,mobile,ipay,resdays,rdatetime from reservedrooms");
    ...
    public RREntity dateStuffSelect(int roomNo) throws Exception{
        preparedStatement=connection.prepareStatement("select  rdatetime,resdays from reservedrooms where roomno=? order by rdatetime desc ");
